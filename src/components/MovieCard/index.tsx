...
//Displays a movie poster and title. Uses Link to navigate to the movie details page when the movie card is clicked
const MovieCard = ({ movie, category }: MovieCardProps) => {
  ...
  return (
    <>
      <Link
        to={`/${category}/${id}`}
        className="dark:bg-[#1f1f1f] bg-[#f5f5f5] rounded-lg relative group select-none xs:h-[250px] h-[216px] overflow-hidden"
      >
        <Image
